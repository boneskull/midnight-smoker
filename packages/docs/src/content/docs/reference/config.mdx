...
_Most_ options have equivalent [command-line flags](/reference/cli). Generally, if the JSON type of an option value is an _object_ (i.e. more complex than `string[]`), the option will _not_ be available on the CLI, because this is cumbersome and awful and I hate it.

:::note[About Config File Option Names]
...
Each rule can have its severity set to one of `error`, `warn`, or `off`. Rules may also define their own options. See the [Rules Reference](/reference/rules) for more information on specific rule options.
